...
// --- NEW, CORRECTED EGO SMS FUNCTION ---

/**
 * Sends an SMS using the EgoSMS Plain Text API.
 * This is a helper function, not directly callable by the client.
 */
async function _sendSmsPlainText(username, password, sender, recipient, message, priority = 0) {
  ...
  const params = new URLSearchParams({
    number: recipient.replace(/\s+/g, ''), // Clean number just in case
    message: message,
    username: username,
    password: password,
    sender: sender,
  ...
  // Log the URL for debugging, masking the password
...
exports.sendSms = onCall({ region: "us-central1", enforceAppCheck: false, timeoutSeconds: 60, memory: '256MiB' }, async (request) => {
    ...
    if (!request.auth) {
      logger.error("Authentication failed: No auth context available for sendSms.");
    ...
    if (!schoolId || !recipient || !message) {
      logger.error("Validation Error: Missing schoolId, recipient, or message.", request.data);
      throw new HttpsError("invalid-argument", "Missing required parameters for sending SMS.");
    }
    
    try {
      const schoolDoc = await db.collection("schools").doc(schoolId).get();
      if (!schoolDoc.exists) {
        logger.error(`School not found: ${schoolId}`);
        throw new HttpsError("not-found", "School document not found.");
      }
      const schoolData = schoolDoc.data();
      if (!schoolData.adminUids || !schoolData.adminUids.includes(adminUid)) {
        logger.error(`Authorization failed: User ${adminUid} is not an admin for school ${schoolId}.`);
        throw new HttpsError("permission-denied", "You are not authorized to perform this action for this school.");
      }

      if (!schoolData.enableSmsNotifications) {
        logger.warn(`SMS notifications are disabled for school ${schoolId}.`);
      ...
      if (!egoSmsConfig?.username || !egoSmsConfig?.password || !egoSmsConfig?.sender) {
        logger.warn(`EgoSMS configuration missing for school ${schoolId}.`);
        return { success: false, message: "EgoSMS Username, Password, or Sender ID is missing in school settings." };
      }
