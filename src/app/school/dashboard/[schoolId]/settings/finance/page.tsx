...
import { storage, firestore, functions as firebaseFunctions } from '@/config/firebase'; // Added functions
...
import { Loader2, Save, DollarSign, FileText, XCircle, PlusCircle, Trash2, Tag, Upload, Settings, Landmark, Edit, Briefcase, Award, AlertTriangle, ChevronDown, ChevronRight } from 'lucide-react';
...
const generalFinanceSettingsSchema = z.object({
  ...
  defaultFeeRevenueAccountId: z.string().min(1, "Default General Fee Revenue account is required.").optional().nullable(),
...
export default function FinanceSettingsPage() {
  ...
  const [showCloseTermConfirm, setShowCloseTermConfirm] = useState(false);
  const [isClosingTerm, setIsClosingTerm] = useState(false);
  const [nextTermIdToClose, setNextTermIdToClose] = useState<string>('');
  ...
  const generalForm = useForm<SchoolSettingsFormData>({
    ...
    defaultValues: {
      ...
      defaultFeeRevenueAccountId: "", // Added
    },
  });
  ...
  const coaForm = useForm<ChartOfAccountItemFormValues>({
    ...
    defaultValues: { accountName: "", accountType: undefined, parentAccountId: null, accountCode: "", description: "" },
  });
    ...
    try {
      ...
      if (fetchedSchool) {
        ...
        generalForm.reset({
          ...
          defaultFeeRevenueAccountId: fetchedSchool.defaultFeeRevenueAccountId || "", // Added field
        });
  ...
  const onSubmitGeneralForm = async (data: SchoolSettingsFormData) => {
    ...
    try {
      ...
      const schoolDataToUpdate: Partial<School> = {
        ...
        defaultFeeRevenueAccountId: data.defaultFeeRevenueAccountId || null, // Added field
      };
  ...
  const openCoADialog = (item?: ChartOfAccountItem) => {
    if (item) {
      ...
      coaForm.reset({ accountName: "", accountType: undefined, parentAccountId: null, accountCode: "", description: "" });
    }
  ...
  const handleCoASubmit = async (data: ChartOfAccountItemFormValues) => {
    ...
    try {
      const coaData: Omit<ChartOfAccountItem, 'id' | 'createdAt' | 'updatedAt' | 'balance' | 'balanceType'> = {
        ...
        parentAccountId: data.parentAccountId === '_NO_PARENT_' ? null : data.parentAccountId || null,
  ...
  const handleCloseFinancialTerm = async () => {
    if (!user || !nextTermIdToClose) {
        toast({ variant: "destructive", title: "Error", description: "You are not authenticated or next term is not selected." });
        return;
    }
    setIsClosingTerm(true);
    setShowCloseTermConfirm(false);
    toast({ title: "Term Closing Process Started...", description: "This may take a moment. Please do not navigate away." });
    
    try {
        const closeTermFunction = httpsCallable(firebaseFunctions, 'closeFinancialTerm');
        const result = await closeTermFunction({ schoolId: schoolId, adminUid: user.uid, nextTermId: nextTermIdToClose });
        const { success, message } = result.data as { success: boolean, message: string };

        if (success) {
            toast({ title: "Financial Term Closed Successfully", description: message, duration: 10000 });
            await fetchInitialPageData(); // Re-fetch all data to reflect changes
        } else {
            throw new Error(message || "An unknown error occurred on the server.");
        }
    } catch (error: any) {
        console.error("Error calling closeFinancialTerm cloud function:", error);
        toast({ variant: "destructive", title: "Term Closing Failed", description: error.message, duration: 10000 });
    } finally {
        setIsClosingTerm(false);
        setNextTermIdToClose('');
    }
  };
  ...
  const possibleNextTerms = useMemo(() => {
    if(!school || !school.currentTerm || !school.currentAcademicYearId) return [];
    
    const currentTermDoc = schoolTerms.find(t => t.academicYearId === school.currentAcademicYearId && t.name === school.currentTerm);
    if (!currentTermDoc || !currentTermDoc.createdAt) return [];

    const currentTermDate = (currentTermDoc.createdAt as Timestamp).toDate();

    return schoolTerms.filter(term => {
        if (term.isClosed) return false;
        const termDate = (term.createdAt as Timestamp)?.toDate();
        if(!termDate) return false;
        return termDate > currentTermDate;
    }).sort((a,b) => ((a.createdAt as Timestamp).toDate().getTime()) - ((b.createdAt as Timestamp).toDate().getTime()));

  }, [school, schoolTerms]);
  
  const [openCoaCategories, setOpenCoaCategories] = useState<Record<string, boolean>>({
    'Asset': true, 'Liability': true, 'Equity': true, 'Revenue': true, 'Expense': true
  });
  
  const coaTree = useMemo(() => {
    const tree: Record<string, { parent: ChartOfAccountItem; children: ChartOfAccountItem[] }> = {};
    const topLevel: ChartOfAccountItem[] = [];

    chartOfAccounts.forEach(item => {
      if (item.parentAccountId) {
        if (!tree[item.parentAccountId]) {
          const parent = chartOfAccounts.find(p => p.id === item.parentAccountId);
          if (parent) {
            tree[item.parentAccountId] = { parent, children: [] };
          }
        }
        if (tree[item.parentAccountId]) {
          tree[item.parentAccountId].children.push(item);
        }
      } else {
        topLevel.push(item);
      }
    ...
    Object.values(tree).forEach(node => {
      node.children.sort((a, b) => (a.accountName || '').localeCompare(b.accountName || ''));
    });
  ...
  const parentAccountsForSelect = (type: AccountType) => chartOfAccounts.filter(acc => acc.accountType === type && !acc.parentAccountId);
  ...
  return (
    <div className="space-y-8">
      <Card className="shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                />
                 <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                        <FormLabel>Default Fee Revenue Account*</FormLabel>
                        <Select onValueChange={field.onChange} value={field.value || ""} disabled={revenueAccountsForSelect.length === 0}>
                            <FormControl><SelectTrigger><SelectValue placeholder="Select Fee Revenue Account" /></SelectTrigger></FormControl>
                            <SelectContent>
                        ...
                        <FormDescription className="text-xs">Default revenue account for un-itemized student payments.</FormDescription>
                        <FormMessage />
                        </FormItem>
                    )}
                />
                    ...
                    <ShadcnDialogContent className="sm:max-w-lg">
                        ...
                        <Form {...coaForm}>
                            <form onSubmit={coaForm.handleSubmit(handleCoASubmit)} className="space-y-4 py-2">
                                ...
                                <FormField control={coaForm.control} name="parentAccountId" render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Parent Account (Optional)</FormLabel>
                                        <Select onValueChange={field.onChange} value={field.value || "_NO_PARENT_"} disabled={!coaForm.watch('accountType')}>
                                            <FormControl><SelectTrigger><SelectValue placeholder="Select a parent account..." /></SelectTrigger></FormControl>
                                            <SelectContent>
                                                <SelectItem value="_NO_PARENT_">No Parent</SelectItem>
                                                {parentAccountsForSelect(coaForm.watch('accountType')!).map(acc => <SelectItem key={acc.id} value={acc.id}>{acc.accountName}</SelectItem>)}
                                            </SelectContent>
                                        </Select>
                                        <FormDescription className="text-xs">Assign this as a sub-account to a main account of the same type.</FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                )}/>
                    ...
                    {chartOfAccounts.length === 0 ? <p className="text-sm text-muted-foreground">No accounts defined yet.</p> : (
                        <div className="max-h-96 overflow-y-auto border rounded-md">
                          {accountTypeOptions.map(type => (
                              <div key={type}>
                                  <button onClick={() => setOpenCoaCategories(prev => ({...prev, [type]: !prev[type]}))} className="w-full flex items-center bg-muted/50 p-2 text-left font-semibold">
                                      {openCoaCategories[type] ? <ChevronDown className="h-4 w-4 mr-2"/> : <ChevronRight className="h-4 w-4 mr-2"/>}
                                      {type} Accounts
                                  </button>
                                  {openCoaCategories[type] && (
                                    <Table>
                                        <TableHeader><TableRow><TableHead>Account Name</TableHead><TableHead>Code</TableHead><TableHead className="text-right">Balance (UGX)</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
                                        <TableBody>
                                            {coaTree.topLevel.filter(acc => acc.accountType === type).map(item => (
                                                <React.Fragment key={item.id}>
                                                  <AccountRow item={item} getAccountBalance={getAccountBalance} openCoADialog={openCoADialog} handleDeleteChartOfAccountItem={handleDeleteChartOfAccountItem} isProcessingCoA={isProcessingCoA} />
                                                  {coaTree.tree[item.id]?.children.map(child => (
                                                    <AccountRow key={child.id} item={child} getAccountBalance={getAccountBalance} openCoADialog={openCoADialog} handleDeleteChartOfAccountItem={handleDeleteChartOfAccountItem} isSubAccount isProcessingCoA={isProcessingCoA} />
                                                  ))}
                                                </React.Fragment>
                                            ))}
                                        </TableBody>
        ...
        <AlertDialog open={showCloseTermConfirm} onOpenChange={setShowCloseTermConfirm}>
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Closing the term for <span className="font-bold">{school?.currentTerm} ({academicYears.find(ay => ay.id === school?.currentAcademicYearId)?.year})</span> and opening <span className="font-bold">{schoolTerms.find(t => t.id === nextTermIdToClose)?.name || 'the next term'}</span> is a permanent action.
                        <ul className="list-disc pl-5 mt-2 space-y-1">
                            <li>It will calculate final balances for all accounts.</li>
                            <li>It will create opening balance journal entries for the next term.</li>
                            <li>The current term will be marked as 'Closed', preventing further transactions.</li>
                        </ul>
                ...
                <AlertDialogFooter>
                    ...
                    <AlertDialogAction onClick={handleCloseFinancialTerm} disabled={isClosingTerm} className="bg-destructive hover:bg-destructive/90">
                        {isClosingTerm ? <Loader2 className="animate-spin mr-2"/> : null}
                        Yes, Close Financial Term
                    </AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
...
// Helper component to render a row in the CoA table, including children
const AccountRow: React.FC<{item: ChartOfAccountItem, getAccountBalance: (id: string) => {balance: number, type: string}, openCoADialog: (item: ChartOfAccountItem) => void, handleDeleteChartOfAccountItem: (id: string, name: string) => void, isSubAccount?: boolean, isProcessingCoA: boolean}> = 
({ item, getAccountBalance, openCoADialog, handleDeleteChartOfAccountItem, isSubAccount = false, isProcessingCoA }) => {
  const { balance, type: balanceType } = getAccountBalance(item.id);
  return (
    <TableRow className={isSubAccount ? "bg-muted/30" : ""}>
        <TableCell className={`font-medium text-sm ${isSubAccount ? 'pl-8' : ''}`}>
          {item.accountName}
        </TableCell>
